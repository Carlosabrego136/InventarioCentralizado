// Sesiones firmadas con HMAC — sin dependencias externas, funciona en
// cualquier runtime de Node (incluido Node 16 en local).
...
export function createSessionCookie(payload) {
  ...
  return `palafox_session=${token}; HttpOnly; Path=/; Max-Age=${MAX_AGE_SECONDS}; SameSite=Lax${secure}`;
}

export function clearSessionCookie() {
  return 'palafox_session=; HttpOnly; Path=/; Max-Age=0; SameSite=Lax';
}
...
export function getSession(req) {
  ...
  return verify(cookies.palafox_session);
}

// Helper para rutas API: valida sesión y responde 401 si no hay.
// Devuelve la sesión, o null (y ya mandó la respuesta de error).
